...
def extracting_drug_fda_publ_date(response):
    ...
    if response['fields']['status'] == 'Done':
        if set(keys_list).issubset(response['fields']['data']['message'].keys()):
            ...
            # for idx in list(range(1434, 1534)):
                # print(idx, edge_list[idx])
                if idx % 20 == 0:
...
def compute_novelty(response):
    ...
    pd.DataFrame(grouped_df).to_excel('DATAFRAME_grouped.xlsx', header=False, index=False)
    merged_df = grouped_df.agg({'edge': ', '.join, 'drug':'first', 'fda status': 'max', 'publications': lambda x: ', '.join(x.astype(str)), 'number_of_publ': 'min', 'age_oldest_pub': 'min' })
    condition = lambda row: row['age_oldest_pub'] if 'nan' not in row['publications'] else 0
    merged_df['age_oldest_pub'] = merged_df.apply(condition, axis=1)
    df = merged_df
    pd.DataFrame(merged_df).to_excel('DATAFRAME_merged.xlsx', header=False, index=False)
